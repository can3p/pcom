...
| T0.2 factories + self-test | `sonnet` | Needs only the schema (`go doc ./pkg/model/core <Type>`, `migrations/`). Owns `pkg/testutil/factory/`. |
...
Coordinator verification is the budget in `docs/plan/README.md`: `make cover-q`
