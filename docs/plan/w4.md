...
- **`docker-compose.yml`**. No service pins a `container_name`, so a second
  ...
  | `objectstore` | `adobe/s3mock:5.2.3`, pinned | 9090 | Picked because it needs the least configuration of any S3-compatible server: `COM_ADOBE_TESTING_S3MOCK_STORE_INITIAL_BUCKETS=pcom-media` creates the bucket, so there is no init container. It accepts any credentials, supports path-style addressing and ACL headers, and keeps files in a named volume at `/s3mockroot` when `COM_ADOBE_TESTING_S3MOCK_STORE_RETAIN_FILES_ON_EXIT=true`. Tests use tommy's S3 listener instead (see R2), which also captures mail and has a read-back API. MinIO no longer publishes maintained community images, and SeaweedFS, Garage and RustFS all need bucket or layout bootstrapping. |
  | `mail` | `can3p/tommy:0.2.0`, pinned (Docker Hub tags have no `v`) | 8811 (UI + API), 8822 (fake vendor ingress) | Mail sink. It answers Mailjet's v3.1 send API, so production's Mailjet sender works against it unmodified. From R2 on, all development and E2E mail goes here, and the console sender is removed. Read mail at `http://localhost:8811/ui/`, or from tests via `GET /api/v1/events?plugin=mail`. tommy also has an S3 listener (port 9000), which the E2E harness and the S3 storage tests use from R2 on. It isn't published here: its catalog is kept in memory ([can3p/tommy#40](https://github.com/can3p/tommy/issues/40)), so development uploads go to `objectstore`, which keeps them on a volume. |
