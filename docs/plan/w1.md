...
| U7 | `pkg/media` (`ValidateImageType`, `HandleUpload` argument checks) | Don't test `storage/local`: R2 deletes it. Don't test S3 here either: R2 adds S3 tests against a tommy container. | cheap | 80% of `upload.go` |
